...
```{r Clustering, fig.height=9, fig.width=9}
# raw Data, leave out YEAR (col. 1) from predictors
library(MASS)
dat <- as.matrix(cbind(RECR=popdata$ADULTS, predictors[, -1]))
print(colnames(dat))
image(cor(dat), col=RColorBrewer::brewer.pal(11,'RdBu'))
dat2 <- dat[ , -grep("NPI_", colnames(dat))] #NPI is off the charts on distance
data.mds <- cmdscale(dist(t(dat2)))
eqscplot(data.mds, type='n')
text(data.mds, labels=rownames(data.mds), cex=0.5)
# Cluster (including NPI this time)
h <- hclust(dist(t(dat2)), method="single")
plot(h, cex=0.5)
```
