...
# names(analdata) <- sub('_JFM', '_Wi', names(analdata))
# names(analdata) <- sub('_AMJ', '_Sp', names(analdata))
# names(analdata) <- sub('_JAS', '_Su', names(analdata))
# names(analdata) <- sub('_OND', '_Fa', names(analdata))
# ##newdata <- cbind(YEAR=analdata$YEAR, SPR.TRN=analdata$SPR.TRN,
# ## No spring transition for now (messes up seasonal panel plots)
# newdata <- cbind(YEAR=analdata$YEAR,
#                  analdata[ , grep('_Wi', colnames(analdata))],
#                  analdata[ , grep('_Sp', colnames(analdata))],
#                  analdata[ , grep('_Su', colnames(analdata))],
#                  analdata[ , grep('_Fa', colnames(analdata))])
...
# analdata <- cbind(YEAR=lag0$`YEAR_FW0`, lag0[ , -1], lag1[ , -1], lag2[ , -1])
# analdata <- analdata[ , order(substr(names(analdata), 1, 3))]
# rm(newdata, lag0, lag1, lag2)
# # Make sure both data sets cover same set of years:
# analdata <- analdata[analdata$YEAR %in% popdata$YEAR, ]
# popdata <- popdata[popdata$YEAR %in% analdata$YEAR, ]
analdata <- analdata[!is.na(analdata$OCN.RCR), ] # Use only years with OCN recruit data.
plot(analdata$YEAR, analdata$OCN.RCR, type='l', fg='grey', col='blue', 
     lwd=2, log='y', xlab='Year of Ocean Entry', ylab='Abundance (1000\`s)', 
...
**TODO:** MOVE to data processing scripts!
...
```{r SelectPredictors}
predictors <- analdata[ , !grepl('OCN.RCR', names(analdata))]  # Whole data set except OCN Recruits
