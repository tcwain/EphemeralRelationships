...
Read in the data. Eliminate MEI series from the environmental data, as that almost duplicates ONI, but correlates more with other variables. Environmental series are restructured to 4 seasons (Win [JFM], Spr [AMJ], Sum [JAS], & Aut [OND]) across all three years of the coho lifecycle: freshwater year (year 0), ocean entry year (year 1), and return year (year 2).
...
# Shift popdata so year is freshwater year (return year - 2)
...
newdata <- cbind(YEAR=envdata$YEAR, SPR.TRN=envdata$SPR.TRN,
                 envdata[ , grep('_JFM', colnames(envdata))],
                 envdata[ , grep('_AMJ', colnames(envdata))],
                 envdata[ , grep('_JAS', colnames(envdata))],
                 envdata[ , grep('_OND', colnames(envdata))])
lag0 <- newdata[1:(nrow(newdata)-2), ]
names(lag0) <- paste0(names(lag0), '_0')
lag1 <- newdata[2:(nrow(newdata)-1), ]
names(lag1) <- paste0(names(lag1), '_1')
lag2 <- newdata[3:(nrow(newdata)), ]
names(lag2) <- paste0(names(lag2), '_2')
envdata <- cbind(YEAR=lag0$`YEAR_0`, lag0[ , -1], lag1[ , -1], lag2[ , -1])
envdata <- envdata[ , order(substr(names(envdata), 1, 3))]
rm(newdata, lag0, lagm1, lagp1)
