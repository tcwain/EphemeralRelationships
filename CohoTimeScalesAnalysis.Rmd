...
```{r tsDotPlot}
tsDotPlot <- function(ssList, selScale, dataYears, gofLabel='LOO Skill', ...) {
  ...
  scales <- sort(unique(as.vector(sapply(ssList, function(X) {X$scale}))))
  ...
  symbols(x=(0:20)/20, y=rep(1,21), circles=c(0.1,1:20)/20,
  ...
  plot(c(min(years), max(years)), c(1, n.pred), type="n", axes=F,
       xlab=paste("End-Year of Interval", sep=""), ylab="", cex.lab=1.5)
  box()
  axis(side=1)
  mtext(paste('(', selScale, '-year intervals)'), side=1, line=3.5, cex=1.0)
  ...
  for (p in pred.names) {
    ...
    if (any(!is.na(ss$gof))) {
      symbols(x=dataYears[ss$time], y=rep(yy, length(ss$time)),
              circles=0.5*ss$gof,
              inches=0.1*max(ss$gof, na.rm=T), add=T,
              fg=ifelse(ss$slp < 0, 'red', 'blue'),
              bg=ifelse(ss$slp < 0, 'red', 'blue'))
    } # if(any(!is.na(...)))
  } # for(p)
  
  # j <- 1  #indexes predictor
  # for (x in colnames(predictors)) {
  #   i <- 1  #indexes year (1 == start of ana.yrs)
  #   for (end.yr in ana.yrs) {
  #     s1 <- surv[match((end.yr-pint+1):end.yr, years), y]
  #     p1 <- predictors[match((end.yr-pint+1):end.yr, years), x]
  #     if (sum(!is.na(s1)&!is.na(p1)) > ceiling(pint/2)) { #half the data available
  #       gfit <- lm(s1 ~ p1)
  #       gsum <- summary(gfit)
  #       res.rsq[i, x] <- max(0, gsum$adj.r.squared)
  #       res.slp[i, x] <- coef(gfit)["p1"]
  #       # Compute 1-step ahead prediction:
  #       p1.next <- predictors[match(end.yr+1, years), x]
  #       if (!is.na(p1.next)) {
  #         tmp <- predict(gfit, newdata=data.frame(p1=p1.next), se=T)
  #         pred.fit[i, x] <- tmp$fit
  #         pred.se[i, x] <- tmp$se.fit
  #         pred.err[i, x] <- tmp$fit - s1[length(s1)]  #predicted - observed
  #       } #if !is.na(predictors...
  #     } #if sum(!is.na(s1...
  #     i <- i + 1
  #   }  #for end.yr
  #   if (any(!is.na(res.rsq[ , x]))) {
  #     symbols(x=ana.yrs, y=rep(n.pred+1-j, length(ana.yrs)),
  #             circles=0.5*res.rsq[ ,x],
  #             inches=0.1*max(res.rsq[ ,x], na.rm=T), add=T,
  #             bg=ifelse(res.slp[ ,x]<0,"red","blue"))
  #   }
  #   if(sum(!is.na(pred.err[,j])) > 2) {
  #     mn.skill[j] <- 1 - mean(pred.err[ , j]^2, na.rm=T) /
  #       var(surv[match(ana.yrs[!is.na(pred.err[,j])], years), y])
  #   }  #if
  #   j <- j+1
  # }  #for x
  # mtext(y, side=3, outer=T, line=2)
} # tsDotPlot()
```

Do plot:

```{r dotPlots, fig.height=6, fig.width=9}
ss.sel <- ss.out[1:10]
