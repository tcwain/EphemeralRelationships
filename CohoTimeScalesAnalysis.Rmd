...
```{r Results, fig.height=5.5, fig.width=5.5}
skillthresh <- 0.00
if (BLACKWHITE) {
...
tsDotPlot <- function(ssList, selScale=NA, dataYears, gofLabel='Skill', 
  ...
  symbols(x=c(x1,x2), y=c(n.pred+1.5, n.pred+1.5) , add=TRUE,
          circles=c(0.5,0.5), inches=c(0.1,0.1), fg="grey20",
          bg=dot.fill[c("neg","pos")])
  text(x=c(x1,x2) + 0.03*diff(range(years)), y=rep(n.pred+1.5, 2),
...
```{r dotPlots, fig.height=7.4, fig.width=7.4}
