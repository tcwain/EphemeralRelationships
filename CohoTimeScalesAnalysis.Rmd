...
```{r ReadData, dev=c('postscript'), fig.height=5, fig.width=7.4}
# Environmental data:
...
#  text(min(ss$time), max(ss$scale) - 0.17 * diff(range(ss$scale)),
  text(pl.lims[1]+0.05*xrange, pl.lims[4]-0.15*yrange,
    ...
    symbols(ss$time, ss$scale,
            circles=dot.scale * sqrt(ss$gof), inches=FALSE, fg='grey20',
...
skillPlot(tst, title='Test', dot.scale=0.5, add.contours = TRUE)
rm(X, Y, tst)
```
...
##  if (max(ss$gof, na.rm=TRUE) > skillthresh) { 
    skillPlot(ss, title=series, dot.scale=1.0, dot.fill=fill.col,
              add.contours = FALSE, xlim=c(1975,2015), ylim=c(5,50)) 
##  } # if
} # for(series)
par(op)
rm(skillthresh, op)
...
tsDotPlot <- function(ssList, selScale=NA, dataYears, gofLabel='Skill', 
  ...
  symbols(x=(0:20)/20, y=rep(1,21), circles=sqrt(c(0.1,1:20)/20),
          bg="grey", fg="grey20", inches=0.1, add=T, cex.axis=1.25)
  axis(1)
  par(op)
