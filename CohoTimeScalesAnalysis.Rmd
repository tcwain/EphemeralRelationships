...
```{r OPIHPlot, fig.height=6, fig.width=9}
...
tsDotPlot <- function(ssList, selScale=NA, dataYears, gofLabel='LOO Skill', ...) {
  ...
  if (!is.na(selScale)) {
    ...
    mtext("(All prior data)", side=1, line=4, cex=1.0)
  } # if(!is.na(selScale))
...
Criteria: had some reasonable longterm skill at some time point (max skill >= 0.1).

```{r ltSkillSummary}
  for (p in names(ss.out)) {
    ss <- ss.out[[p]]
    ss <- ss[!is.na(ss$gof), ] # select times and scales with data
      # use all data up to year (the first dot in each row of the plots above)
    ...
    cat(p, ': range(skill) = ', round(range(xx$gof, na.rm=TRUE), 1), '\n')
  } # for(p)
```

The resulting list of those with long-term skill <= 0.1:
...
selpred <- c("PDO.JFM", "PDO.AMJ", "PDO.JAS", "PDO.OND", "ONI.OND",
             "UWI.JAS", "UWI.OND", "CWT.JFM", "CWT.JAS", "CWT.OND", 
             "NPG.AMJ", "NPG.JAS", "NPG.OND", "SPT.LGR", "SPT.BIO", 
             "COP.RCH", "COP.NAN", "COP.SAN", "TMP.DP", "ICH.BIO")
